...
function generateQuote(event) {
  ...
  let context =
    "You are a wise and spiritual quote expert and love to write about short quotes. Your mission is to generate a one line spiritual quote poem in basic HTML format. Make sure to follow the user instructions. Do not include a title to the quote. Sign the quote with 'SheCodes AI' inside a <strong> element at the end of the quote";

  let apiURL = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  let quoteElement = document.querySelector("#quote");
  quoteElement.classList.remove("hidden");
  quoteElement.innerHTML = `<div class="generating">
      ⏳ Generating a spiritual quote about ${instructionInput.value}
    </div>`;
